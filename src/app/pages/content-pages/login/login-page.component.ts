  ...
  onSubmit() {
    ...
    this.auth
      ...
      .then((res: any) => {
          ...
          const userGroup = this.http.getUserGroup();
          if (userGroup && userGroup !== 'undefined') {
            this.http.getData('usergrouprights?filter=groupid=' + userGroup).then(menu=>{
              res['UserMenu'] = menu
              localStorage.setItem('currentUser', JSON.stringify(res));
              this.router.navigate([this.returnUrl]);
            }).catch(err => {
              console.error('Error fetching user group rights:', err);
              this.router.navigate([this.returnUrl]);
            });
          } else {
            // No user group defined, proceed without menu
            this.router.navigate([this.returnUrl]);
          }
          //   setTimeout(() => {
